...
- [~] **Step 6** — REOPENED 2026-06-14. Real intent = **"generate once, cache forever."** A one-time job generates the FULL bank (6 sections × ~20 topics × 20 Qs = ~2,400 questions) into **Firestore**; users then get random DB reads from a real curated bank (NOT live per-user AI, which feels fully-AI). The live per-user generator I shipped (6a below) is REPURPOSED as the Pro "custom topic" power feature. Still TODO: generation job + Firestore bank + client read/cache/serve. Tiers: Elite=full bank unlimited; Pro=20/section/24h (now free DB reads) + custom topic gen; Free=none.
  - CACHED-BANK INFRA SHIPPED 2026-06-14 (Sonnet 4.6, Vercel endpoint): `api/generate-bank.js` (admin, `?secret=&section=&from=&count=`, 20 topics/section hardcoded, writes `questionBank/{SECTION}/topics/{slug}` + meta doc); firestore.rules adds questionBank read; client `loadQuestionBank()` reads+caches per session, `openQuizModal` now async, pulls RANDOM from bank (falls back to static QB_QUESTIONS until populated), tier caps (Free 5 / Pro 20/section/24h via `_aiQbConsume` / Elite full). **PENDING USER (manual): (1) add `BANK_ADMIN_SECRET` env var in Vercel + redeploy, (2) publish updated firestore.rules. THEN I trigger generation 12× (2 chunks × 6 sections).**
...
- (no Step 9 — confirmed by user there never was one)
